...
class t_bts():
        ...
        while y_stack:
                ...
                for sc in events_2c:
                    ...
                    for policy_t in sc_t_current:
                           for policy_index in list(policy_t):
                               ...
                               if t_i < sc_t_min[event_t] or t_j > sc_t_max[event_t]:
            ...
            for policy_t in sc_timed:
                    ...
                    if is_z_state_listed:
                        ...
                        # for debugging
                        if z_state_prime == (('2', '3', '6', '8'), (('a', (2, 4)), ('b', (7, 11)), ('o3', (5, 7)))):
                            # conjunction of policies错了
                            # nx.dijkstra_path(self.t_bts, ('8',), (('2', '8'), (('a', (4, 5)), ('b', (11, 16)), ('o3', (11, 13)))))
                            print(233)
                        ...
                        # for debugging
                        if z_state_extended == (('2', '8'), (('a', (4, 5)), ('b', (11, 16)), ('o3', (11, 13)))):
                            print(233)

                        # 可能错了
                        # z_state_prime == z_state_extended?
                        ...
                        self.t_bts.add_edge(root_state, z_state_t)
    ...
    def get_policy_w_time(self, state):
        ...
        for policy_t in state[1]:
            policy.update({policy_t[0] : [policy_t[1][0], policy_t[1][1]]})
    ...
    def conjunction_of_policies(self, policy_1, policy_2):
        ...
        policy_dict_1 = {}
        for policy_t in policy_1:
            policy_dict_1.update({policy_t[0] : [policy_t[1][0], policy_t[1][1]]})
        policy_dict_2 = {}
        for policy_t in policy_2:
            policy_dict_2.update({policy_t[0] : [policy_t[1][0], policy_t[1][1]]})

        policy_dict = {}
        for event_t in policy_dict_1.keys():
            if event_t in policy_dict_2.keys():
                # if the event is shared
                t_min_1 = policy_dict_1[event_t][0]
                t_max_1 = policy_dict_1[event_t][1]
                t_min_2 = policy_dict_2[event_t][0]
                t_max_2 = policy_dict_2[event_t][1]
                ...
                if (t_min_1 <= t_min_2 and t_min_2 <= t_max_1) or \
                   (t_min_2 <= t_min_1 and t_min_1 <= t_max_2):
                    policy_dict.update({event_t: [min(t_min_1, t_min_2), max(t_max_1, t_max_2)]})
                else:
                    policy_dict.update({event_t: [t_min_1, t_max_1]})
                    policy_dict.update({event_t: [t_min_2, t_max_2]})

            else:
                # if the event is NOT shared, then directly add it to target dict
                policy_dict.update({event_t: [policy_dict_1[event_t][0], policy_dict_1[event_t][1]]})

        # check events in policy_2 again in case for missing
        for event_t in policy_dict_2.keys():
            if event_t not in policy_dict_1.keys():
                # if the event is NOT shared, then directly add it to target dict
                policy_dict.update({event_t: [policy_dict_2[event_t][0], policy_dict_2[event_t][1]]})


        # policy dict -> tuple
        for event_t in policy_dict.keys():
            policy.append((event_t, (policy_dict[event_t][0], policy_dict[event_t][1])))
    ...
    def is_event_in_controlable_observable_policies_identical(self, policy, policy_prime):
        ...
        for event_t in policy.keys():
            if (event_t in self.event_c and event_t in self.event_o) and \
                event_t in policy_prime.keys() and \
               (policy[event_t][0] == policy_prime[event_t][0] and policy[event_t][1] == policy_prime[event_t][1]):
                is_identical = True
    ...
    def replace_node_in_bts(self, node, node_prime):
        ...
        for node_t in in_states:
            self.t_bts.add_edge(node_t, node_prime)
        # add connections for out nodes
        for node_t in out_states:
            self.t_bts.add_edge(node_prime, node_t)
    ...
    def find_root_state_for_z_states(self, current_y_state, z_state):
                    ...
                    if nx.dijkstra_path_length(self.t_bts, current_y_state, state_t) >= 0:                  # 这步错了（可能）, 如果current_y_state -> state_t不通, 则不属于同一y-state, 有没有更好的办法
                        ...
                        if event_state_t.issubset(event_z_state):
                            ...
                            for event_t in policy_state_t_dict.keys():
                                t_min_1 = policy_state_t_dict[event_t][0]
                                t_max_1 = policy_state_t_dict[event_t][1]
                                t_min_2 = policy_z_state_dict[event_t][0]
                                t_max_2 = policy_z_state_dict[event_t][1]
        ...
        if root_state_candidate.__len__():
            ...
            for state_t in root_state_candidate:
                ...
                time_t = 0
                for event_t in policy_state_t_dict.keys():
                    time_t += policy_state_t_dict[event_t][1]       # maximal time
