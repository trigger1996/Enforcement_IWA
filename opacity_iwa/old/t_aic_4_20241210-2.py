...
class t_bts():
            ...
            for neighbor in G.neighbors(v):
                for edge in G.get_edge_data(v, neighbor):  # 遍历所有从v到neighbor的边
                    ...
                    if neighbor not in path:
                        ...
                        dfs(neighbor, path, cost + edge_cost)  # 累加 t_max 作为代价
                        path.pop()  # 回溯
    ...
    def longest_path_with_cost(self, G, start_node, end_node, accessible_events=None):
        ...
        # 深度优先搜索递归函数，计算路径的代价
        def dfs(v, path, cost):
            nonlocal max_cost, longest_path
    ...
    def dfstree2(self, iwa, event_list, event_uc, event_uo, source, t_cutoff=20):
        edges = list(self.dfs_edges2(iwa, event_list, event_uc, event_uo, source, cutoff_weight=t_cutoff))      # [ (start, end, t_min_accumulated, t_max_accumulated, ), ... ]
        ...
        for edge_t in edges:
            ...
            if min_cost_prime > t_cutoff:
                min_cost_prime = t_cutoff
                l_attr = 'closed'
            if max_cost_prime > t_cutoff:
                max_cost_prime = t_cutoff
                r_attr = 'open'
            ...
            if event_st_t in self.event_o:
                ...
                if not dfs_tree.node.__len__():                                                                 # 其实和dfs_tree是一个东西
                    ...
                    try:
                        ...
                        shortest_path_t, t_min_t = self.shortest_path_with_cost(self.iwa, current_state, state_to_calculate, accessible_events)
                        longest_path_t, t_max_t = self.longest_path_with_cost(self.iwa, current_state, state_to_calculate, accessible_events)
                        t_min_t = t_min_t + edge_st[2]['t_min']
