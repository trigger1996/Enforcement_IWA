...
class t_bts():
    ...
    # main function
    def construct_T_BTS(self, source=None, event_uo=None, event_o=None, event_c=None, event_uc=None, t_cutoff=15):
        ...
        while y_stack:
            ...
            print_c("[TAC2024.R2] number of states W_AIC %d" % (self.t_bts.node.__len__(),), color="green", style="bold")
    ...
    def dfs_edges2(self, G, event_list, event_uc, event_uo, source=None, depth_limit=None, cutoff_weight=20):
        ...
        visited = set()
        if depth_limit is None:
            depth_limit = len(G)                            # TODO to remove
        for start in nodes:
            if start in visited:
                continue
            visited.add(start)
            stack = [(start, depth_limit, 0, 0, iter(G[start]))]
            while stack:
                parent, depth_now, t_acc_min, t_acc_max, children = stack[-1]
                try:
                    child = next(children)
                    ...
                    if (G.edge[parent][child][0]['event'] in event_list and G.edge[parent][child][0]['event'] in event_uo) or \
                        ...
                        if True:                                     # 2024.5.14
                            ...
                            yield parent, child, t_acc_min, t_acc_max                       # yield parent, child 这个版本的python没法调试yield  https://zhuanlan.zhihu.com/p/268605982
                            visited.add(str([parent, child, t_acc_min, t_acc_max]))         # visited.add(child)

                            if depth_now > 1 and t_acc_min <= cutoff_weight:
                                stack.append((child, depth_now - 1, t_acc_min, t_acc_max, iter(G[child])))
    ...
    def dfstree2(self, iwa, event_list, event_uc, event_uo, source, is_accumulate_cost=True, t_cutoff=20):
        edges = list(self.dfs_edges2(iwa, event_list, event_uc, event_uo, source, cutoff_weight=t_cutoff))
                    ...
                    while True:
                        ...
                        for i in range(0, loop_t.__len__() - 1):
                            ...
                            if t_min_plus_k > t_cutoff:
                                ...
                                t_max_plus_k = min(t_max_plus_k, t_cutoff)
        ...
        # if source == '3':
        #     if event_list == ['a', 'b', 'o3']:
        #         print_c("dfstree2 / after while: q0: %s, events %s / edges_obtained: %d" % (source, str(event_list), dfs_tree.edges().__len__(),), color='bright_blue', style='underline')
